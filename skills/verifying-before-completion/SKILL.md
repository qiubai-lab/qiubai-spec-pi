...
确认验收后，调用共享 Pi-native [change tool](../closing-qb-change/references/change-tool.md) 中的 `qb_spec_archive` 执行归档，仍在本步骤完成；不因此额外调用 closing skill。工具冲突或中断时使用 `qb_spec_doctor` 诊断并进入显式恢复路径，不能以 shell 或直接编辑自动降级重试。

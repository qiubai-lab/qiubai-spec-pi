...
文件操作优先使用 [change tool](references/change-tool.md)；读取该参考判断 Python 可用性、调用接口和恢复边界。仅运行时不可用时使用下列系统工具逻辑，脚本执行失败不自动降级。
...
3. 解析 `docs/qb-spec/archive/YYYY/<change-id>/`。如果目标已存在、ID 冲突或来源不唯一，停止并请求用户处理，不覆盖。
4. 只有冲突已解决或显式关闭安全时才创建归档目录，将 spec 保存为 `spec.md`；存在 plan 时保存为 `plan.md`，并将 spec 状态设置为 `archived`。
